...
export default function registerGetProjects(
  ...
  server.tool(
    ...
    'Retrieve paginated list of projects for an organisation',
    {
      ...
      textSearch: z.string().optional().describe('Text search filter for project name'),
      projectDescription: z.string().optional().describe('Filter by project description'),
    },
    async ({ orgId, page, pageSize, textSearch, projectDescription }) => {
      try {
        ...
        return handleApiResponse(data, `retrieve projects for organization ${orgId}`, [
