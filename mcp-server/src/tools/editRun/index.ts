...
export default function registerEditRun(
  ...
  server.tool(
    ...
    'Edit an existing test run',
    {
      ...
      runName: z.string().min(1).optional().describe('New run name'),
      runDescription: z.string().optional().describe('New run description'),
    },
    async ({ runId, projectId, runName, runDescription }) => {
      try {
        ...
        return handleApiResponse(data, `edit run ${runId}`, [
