 ...
 */
export function formatErrorResponse(message: string, details?: any): ToolResponse {
...
export function isEmptyData(data: any): boolean {
  ...
  if (typeof data === 'object' && 'data' in data) {
    const d = data.data;
    return !d || (Array.isArray(d) && d.length === 0) || (typeof d === 'object' && Object.keys(d).every(k => d[k] === null));
  }
  return false;
}
...
export function extractApiError(data: any): string {
  if (!data) return 'No response received from server';
  ...
  if (data.status >= 400) {
    ...
    return data.error ? `${statusMessage}: ${data.error}` : statusMessage;
  }
...
export function formatUserFriendlyError(
  ...
  } else if (errorDetails.includes('Locked') || data.status === 423) {
    message += `💡 Solution:\n`;
  ...
  } else if (errorDetails.includes('Conflict') || data.status === 409) {
    message += `💡 Solution:\n`;
