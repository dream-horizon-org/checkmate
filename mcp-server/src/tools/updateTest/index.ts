...
export default function registerUpdateTest(
  ...
  server.tool(
    ...
    'Update an existing test case',
    {
      ...
      testName: z.string().min(1).optional().describe('New test name'),
      testDescription: z.string().optional().describe('New test description'),
      sectionId: z.number().int().positive().optional().describe('Section ID'),
      priorityId: z.number().int().positive().optional().describe('Priority ID'),
      typeId: z.number().int().positive().optional().describe('Test type ID'),
      automationStatusId: z.number().int().positive().optional().describe('Automation status ID'),
      platformId: z.number().int().positive().optional().describe('Platform ID'),
      testCoveredById: z.number().int().positive().optional().describe('Test covered by ID'),
      squadId: z.number().int().positive().optional().describe('Squad ID'),
      labels: z.array(z.number().int().positive()).optional().describe('Array of label IDs'),
    },
    async (params) => {
      try {
        ...
        return handleApiResponse(data, `update test ${params.testId}`, [
